...
public interface ReservationRepository extends JpaRepository<Reservation, Long> {
    ...
    @Query("SELECT COUNT(r) > 0 FROM Reservation r WHERE r.room = :room AND (r.status = 'APPROVED' OR r.status = 'REQUESTED') AND r.reservationStartTime < :endTime AND r.reservationEndTime > :startTime")
